...
### Structure des fichiers Maven

```
injection-dependances/
├── pom.xml                              ← Dépendances Maven (Spring 6)
├── config.txt                           ← Utilisé par la Version 2 (réflexion)
├── README.md
└── src/
    └── main/
        ├── java/net/ayoub/
        ...
        │   └── presentation/
        │       ├── Presentation1.java   ← Version 1 : Instanciation Statique
        │       ├── Presentation2.java   ← Version 2 : Réflexion Dynamique
        │       ├── Presentation3.java   ← Version 3 : Spring XML
        │       └── Presentation4.java   ← Version 4 : Spring Annotations
        └── resources/
...
**`config.txt`** (à la racine du projet) :
```
net.ayoub.dao.DaoImpl
net.ayoub.metier.MetierImpl
```
...
// Injection par constructeur via réflexion
...
<!-- Alternative : Injection par Constructeur -->
<!-- <bean id="metier" class="net.ayoub.metier.MetierImpl">
...
System.out.println(metier.calcul()); // bean déjà injecté !
```
...
```java
@Autowired
public MetierImpl(@Qualifier("dao") IDao dao) { // injecte DaoImpl
    this.dao = dao;
}
// Pour utiliser DaoImplV2 : @Qualifier("daoV2")
```

**Présentation** :
```java
@Configuration
@ComponentScan("net.ayoub") // Spring scanne ce package
